...
The source response exposed the following fields; sentiment analysis needs
comment text and a stable row identity, not a complete author profile:

| Pointer, for each array item `i` | Type | Behavior |
|---|---|---|
| `/i/actor/name` | string | Name as supplied; optional for sentiment |
...
| `/i/commentary` | string | Comment text; missing/empty excludes from sentiment classification |
